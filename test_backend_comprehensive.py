...
def test_stats_formatting():
    ...
    assert uptime_str == "01:01:01", f"Uptime format incorrect: {uptime_str}"
...
def test_script_syntax(script_name):
    ...
    result = subprocess.run(["bash", "-n", script_path], check=False, capture_output=True)
...
def test_iphone_script_features():
    ...
    with open("/app/pdanet-iphone-connect") as f:
        content = f.read()
...
def test_wifi_stealth_features():
    ...
    with open("/app/scripts/wifi-stealth.sh") as f:
        content = f.read()
