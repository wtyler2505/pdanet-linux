...
class TestIPTablesIntegration(unittest.TestCase):
    ...
    def test_interface_detection(self, mock_run):
        ...
        result = subprocess.run(
            ["ip", "link", "show"], check=False, capture_output=True, text=True, timeout=30
        )
...
"""
    ...
    def test_proxy_connectivity_check(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="HTTP/1.1 200 OK\r\n\r\n", stderr="")
...
class TestCarrierBypassIntegration(unittest.TestCase):
    ...
    def test_ipv6_disabled_verification(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, stdout="", stderr="curl: (7) Couldn't connect to server"
        )
    ...
    def test_dns_leak_prevention(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="Server:\t\t192.168.43.1\nAddress:\t192.168.43.1#53\n", stderr=""
        )

        result = subprocess.run(
            ["nslookup", "google.com"], check=False, capture_output=True, text=True, timeout=30
        )
...
class TestConnectionScriptIntegration(unittest.TestCase):
    ...
    def test_usb_connection_script(self, mock_run):
        ...
        result = subprocess.run(
            ["sudo", "./pdanet-connect"], check=False, capture_output=True, text=True, timeout=60
        )
