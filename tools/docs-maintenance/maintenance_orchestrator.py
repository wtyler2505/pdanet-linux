...
class DocumentationMaintenanceOrchestrator:
    ...
    async def run_task(self, task: MaintenanceTask) -> TaskResult:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                'python3', str(script_path), str(self.project_root),
    ...
    def _find_latest_report(self, task_name: str) -> Optional[Path]:
    ...
    async def run_maintenance(self, task_filter: Optional[List[str]] = None,
                            force_all: bool = False) -> MaintenanceReport:
    ...
    def _sort_tasks_by_dependencies(self, tasks: List[MaintenanceTask]) -> List[MaintenanceTask]:
        ...
        while remaining_tasks:
            ...
            for task in remaining_tasks:
                resolved_deps = [dep for dep in task.dependencies
    ...
    def _generate_recommendations(self, task_results: List[TaskResult]) -> List[str]:
        ...
        if failed_tasks:
            recommendations.append(f"Fix {len(failed_tasks)} failed tasks before next maintenance run")

        if timeout_tasks:
            recommendations.append(f"Increase timeout for {len(timeout_tasks)} tasks that timed out")

        if slow_tasks:
            recommendations.append(f"Optimize {len(slow_tasks)} slow-running tasks for better performance")
        ...
        if not recommendations:
            recommendations.append("All tasks completed successfully. Documentation maintenance is up to date.")
...
async def main():
    ...
    parser.add_argument('--force-all', action='store_true', help='Run all tasks including disabled ones')
