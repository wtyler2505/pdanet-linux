...
class DocumentationStyleChecker:
    ...
    def _check_markdown_specific(
        ...
        table_lines = [
            i for i, line in enumerate(lines) if "|" in line and not line.strip().startswith("```")
        ]
    ...
    def check_all_files(self) -> StyleReport:
        ...
        statistics = {
            ...
            "most_common_issues": dict(
                Counter(issue.issue_type for issue in all_issues).most_common(5)
            ),
        }
